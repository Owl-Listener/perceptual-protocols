...
A protocol for translating audio into perceptual brief. Take a soundtrack, an ambient recording, a piece of music, a voice, a place — any sound source that carries the feeling of a project — and produce a `listen.md` an agent can read to inform visual, motion, or experiential output.
...
1. Gather your audio inputs. The protocol is deliberately flexible about format:
   - **Audio files** (MP3, WAV) when the model supports them
   - **Streaming links** (YouTube, Spotify, Bandcamp) — even when the model can't actually "hear" them, it can often identify the work culturally and reason from training knowledge
   - **Named references** (*"Brian Eno – Music for Airports"*, *"the soundtrack to Lost in Translation"*) — works famously enough to be recognised
   - **Written descriptions** of soundscapes (*"a small independent bookshop on a Saturday afternoon — low chatter, the rustle of pages, distant traffic, a kettle in the back"*)
   - **Spectrograms** — visual representations of audio, sometimes the cleanest way to get sonic information into a vision model
...
- **Best case:** the model handles your audio file directly and reads it acoustically. Available with some models, variable quality.
- **Cultural case:** you reference a famous work by name (*"Music for Airports"*, *"the Twin Peaks theme"*, *"Bach's Cello Suite No. 1"*), and the model reasons from its training knowledge of how that work sounds and what it culturally signifies. This is honest but anchored in *received perception*, not *direct listening*.
...
The PROMPT.md explicitly invites combinations. Don't fight the tool — use whatever fidelity is available and document which it is so the brief is read with the right caveats.
